...
public enum RpcError {
    ...
    private static final Map<Integer, RpcError> RPC_ERROR_MAP = new HashMap<>();

    static {
        for (final RpcError error : RpcError.values()) {
            RPC_ERROR_MAP.put(error.getNumber(), error);
        }
    }
    ...
    public static RpcError forNumber(final int value) {
        return RPC_ERROR_MAP.getOrDefault(value, UNKNOWN);
    }

    public static String describeCode(final int code) {
        RpcError e = forNumber(code);
        return e != null ? e.name() : "<Unknown:" + code + ">";
    }

    private final int value;

    RpcError(final int value) {
        this.value = value;
    }
}