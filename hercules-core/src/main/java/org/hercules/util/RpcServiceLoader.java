 ...
 * @author jiachun.fjc
 */
public final class RpcServiceLoader<S> implements Iterable<S> {

    private static final Logger      LOG       = LoggerFactory.getLogger(RpcServiceLoader.class);
    ...
    public static <S> RpcServiceLoader<S> load(final Class<S> service) {
        return RpcServiceLoader.load(service, Thread.currentThread().getContextClassLoader());
    }

    public static <S> RpcServiceLoader<S> load(final Class<S> service, final ClassLoader loader) {
        return new RpcServiceLoader<>(service, loader);
    }
    ...
    private RpcServiceLoader(final Class<S> service, final ClassLoader loader) {
    ...
    public Iterator<S> iterator() {
        return new Iterator<S>() {
            ...
            final Iterator<Map.Entry<String, S>> knownProviders = RpcServiceLoader.this.providers.entrySet()
                                                                    .iterator();

            @Override
            public boolean hasNext() {
                return this.knownProviders.hasNext() || RpcServiceLoader.this.lookupIterator.hasNext();
            }
            ...
            public S next() {
                ...
                final Class<S> cls = RpcServiceLoader.this.lookupIterator.next();
                return newProvider(cls);
            }
    ...
    public Iterator<Class<S>> classIterator() {
        return new Iterator<Class<S>>() {
            ...
            public Class<S> next() {
                ...
                return RpcServiceLoader.this.lookupIterator.next();
            }
    ...
    public String toString() {
        return "com.alipay.sofa.hercules.util.RpcServiceLoader[" + this.service.getName() + "]";
    }
}
